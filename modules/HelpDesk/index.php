...
class HelpDesk extends BaseModule
{
	...
	function detail($targetid)
	{
		...
		if (isset($_FILES['customerfile']))
			$data['errors'] = $uploadres = $this->add_attachment();
		if (AppRequest::has('comments'))
			$comm = $this->update_comment();
		if (AppRequest::get('fun') == 'close_ticket')
			$cl = $this->close_ticket($ticketid);
		...
		foreach ($ticketinfo as $ticketfield) {
			...
			if (!isset($data['ticket_infos'][$blockname]))
				$data['ticket_infos'][$blockname] = [];
			...
			} else if ($fieldlabel == 'Ticket No')
				$data['ticketno'] = $fieldvalue;
		}

		$data['attachments'] = $this->get_ticket_attachments_list(AppRequest::get('id'));
	...
	function create_new()
	{
		...
		if (isset($result[0]) && count($result[0]) > 0 && $result[0] != "")
			foreach ($result[0] as $key => $pick) {
		...
		if (!AppRequest::isEmpty('title')) {
			...
			foreach ($ticket as $key => $val)
				$ticket[$key] = AppRequest::get($key);
			$title = AppRequest::get('title');
			$description = AppRequest::get('description');
			$priority = AppRequest::get('priority');
	...
	function update_comment()
	{
		...
		$comments = AppRequest::get('comments');
		$customerid = $_SESSION["loggeduser"]['id'];
		$sessionid = $_SESSION["loggeduser"]['sessionid'];
	...
	function add_attachment()
	{
		...
		if (AppRequest::has('customerfile_hidden')) {
			$filename = AppRequest::get('customerfile_hidden');
		}
